...
func (wc *WorkoutCreate) check() error {
	...
	if v, ok := wc.mutation.Name(); ok {
		if err := workout.NameValidator(v); err != nil {
			return &ValidationError{Name: "name", err: fmt.Errorf(`ent: validator failed for field "Workout.name": %w`, err)}
		}
	}
	if _, ok := wc.mutation.CreatedAt(); !ok {
...
func (wc *WorkoutCreate) createSpec() (*Workout, *sqlgraph.CreateSpec) {
	...
	if value, ok := wc.mutation.UserID(); ok {
		_spec.SetField(workout.FieldUserID, field.TypeUUID, value)
		_node.UserID = value
	}
