...
func (h *WorkoutLogHandler) StoreWorkoutLog(c echo.Context) error {
	...
	// No need to validate if req.WorkoutID is required anymore due to omitempty
	...
	// 1. Fetch the workout template and its associated workout exercises (ONLY if WorkoutID is provided)
	...
	if req.WorkoutID != nil { // Only execute this block if WorkoutID is provided
		query := `
			SELECT
		...
		foundWorkoutTemplateDetails := false // Flag to ensure we populate workout details only once
		...
		// If WorkoutID was provided but no template was found (e.g., invalid ID or not accessible)
		if !foundWorkoutTemplateDetails {
	...
	// Assign WorkoutID only if it was provided in the request
	if req.WorkoutID != nil {
		workoutLog.WorkoutID = req.WorkoutID
	}

	insertWorkoutLogQuery, args, buildErr := h.sq.Insert("workout_logs").SetMap(map[string]interface{}{
		"id":                            workoutLog.ID,
		"workout_id":                    workoutLog.WorkoutID, // This will be nil if not provided
	...
	// 3. Create logged_exercise_instances and ExerciseSets ONLY if WorkoutID was provided and exercises were found
	...
	currentWorkoutLogFetched := false
	...
	for finalRows.Next() {
		...
		if !currentWorkoutLogFetched {
			...
			if wlWorkoutID.Valid {
				...
				// If no workout_id was provided, set to a zero UUID or handle as appropriate for your DTO.
				// Based on your DTO, WorkoutID is non-nullable, so setting to uuid.Nil is a valid approach.
				finalWorkoutLog.WorkoutID = uuid.Nil
			}
			...
			finalWorkoutLog.LoggedExerciseInstances = []dto.LoggedExerciseInstanceLog{}
			...
			if wID.Valid {
				...
				// If no workout template was linked, provide a default or empty WorkoutResponse
				// so the JSON structure is consistent.
				// Based on your DTO, WorkoutResponse is a value, not a pointer, so it will always exist.
				// If workoutTemplateID is valid (meaning a template was found), use its details.
				// Otherwise, initialize with zero values or a meaningful default.
				if req.WorkoutID != nil && workoutTemplateID != uuid.Nil { // Check if a template was actually attempted to be fetched
					finalWorkoutLog.Workout = dto.WorkoutResponse{
					...
					// No workout template was provided or found. Initialize with zero values.
					finalWorkoutLog.Workout = dto.WorkoutResponse{}
				}
			}
		...
		// Process LoggedExerciseInstances and ExerciseSets only if they exist from the query
		if leiID.Valid {
			leiUUID := leiID.V
			...
			if !exists {
				...
				leiPointerMap[leiUUID] = leiPointer
			}
